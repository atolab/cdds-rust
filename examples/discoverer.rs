...
pub enum MatchedEntity {
    DiscoveredPublication {name: String, partition: Option<String>},
    UndiscoveredPublication {name: String, partition: Option<String>},
    DiscoveredSubscription {name: String, partition: Option<String>},
    UndiscoveredSubscription {name: String, partition: Option<String>}
}
            ...
            if has_partition {
                for k in 0..n {
                    ...
                    if si[i as usize].instance_state == dds_instance_state_DDS_IST_ALIVE {
                        if btx.0 {
                            (btx.1).send(MatchedEntity::DiscoveredPublication { name: String::from(topic_name), partition: Some(String::from(p)) } ).unwrap();
                        } else {
                            (btx.1).send(MatchedEntity::DiscoveredSubscription { name: String::from(topic_name), partition: Some(String::from(p)) } ).unwrap();
                        }
                    }
                    else if btx.0 {
                        (btx.1).send(MatchedEntity::UndiscoveredPublication { name: String::from(topic_name), partition: Some(String::from(p))} ).unwrap();
                    } else {
                        (btx.1).send(MatchedEntity::UndiscoveredSubscription { name: String::from(topic_name), partition: Some(String::from(p)) } ).unwrap();
                    }
                }
            }
            else if si[i as usize].instance_state == dds_instance_state_DDS_IST_ALIVE {
                if btx.0 {
                    (btx.1).send(MatchedEntity::DiscoveredPublication { name: String::from(topic_name), partition: None } ).unwrap();
                } else {
                    (btx.1).send(MatchedEntity::DiscoveredSubscription { name: String::from(topic_name), partition: None }).unwrap();
                }
            }
            else if btx.0 {
                (btx.1).send(MatchedEntity::UndiscoveredPublication { name: String::from(topic_name), partition: None} ).unwrap();
            } else {
                (btx.1).send(MatchedEntity::UndiscoveredSubscription { name: String::from(topic_name), partition: None } ).unwrap();
            }
        }
    }
