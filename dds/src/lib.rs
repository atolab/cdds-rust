    ...
    fn history(&mut self, h: &History) {
        match h {
            History::KeepLast { n } => unsafe {
                dds_qset_history(self.qos, dds_history_kind_DDS_HISTORY_KEEP_LAST, *n as i32)
            },
            History::KeepAll => unsafe {
                dds_qset_history(self.qos, dds_history_kind_DDS_HISTORY_KEEP_ALL, 0)
            },
        }
    }
